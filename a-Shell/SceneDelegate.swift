...
@objc func generateToolbarButtons() {
    ...
    for urlContext in openURLContexts {
            ...
            if ((fileURL.scheme ?? "").hasPrefix("ashell")) {
                NSLog("We received an URL in openURLContexts: \(fileURL.absoluteString.removingPercentEncoding ?? "")") 
                ...
                let isHeadless = fileURL.absoluteString.contains("headless=true")

                if (UIDevice.current.model.hasPrefix("iPad") && !isHeadless) {
                    // Standard iPad behavior: open new window
                    let activity = NSUserActivity(activityType: "AsheKube.app.a-Shell.ExecuteCommand")
                    activity.userInfo!["url"] = fileURL
                    UIApplication.shared.requestSceneSessionActivation(nil, userActivity: activity, options: nil)
                } else {
                    // iPhone behavior or Headless mode
                    installQueue.async {
                        if let groupUrl = FileManager().containerURL(forSecurityApplicationGroupIdentifier:"group.AsheKube.a-Shell") {
                            changeDirectory(path: groupUrl.path)
                        }
                        ...
                        // If headless, send to background after a short delay to ensure execution started
                        if isHeadless {
                            DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                                // This is the standard way to programmatically minimize an iOS app
